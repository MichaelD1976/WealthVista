...
def main():
    ...
    st.header('Portfolio Drawdown Projections', divider='blue')
    ...
    with col1:
        with st.container(border=True):
            ...
            annual_withdrawals = st.number_input(f'Enter Annual Portfolio Withdrawal {denom}', min_value = 0, value = 35000)
            annual_withdrawals = annual_withdrawals * -1
    ...
    if (stocks_weight + bonds_weight + property_weight + cash_weight + btc_weight == 100) and current_value > 0:
        ...
        for year in range(1, num_years + 1):
            ...
            year_data['Withdrawal'] = annual_withdrawals
            
            start_value += lump_sum + annual_withdrawals
            ...
            # Calculate end value
            year_data['End Value'] = start_value + total_growth
    ...
    # df_projection['End Value'] = df_projection['End Value']
    ...
    # Ensure values are clipped before plotting
    df_long['Value'] = df_long['Value'].clip(lower=0)                       # ensure nothing below 0 is shown
    df_cumulative['Total'] = df_cumulative['Total'].clip(lower=0)
    ...
    fig.update_layout(
        ...
        height=600,
        plot_bgcolor='rgba(95, 95, 95, 0.95)',  # Darker grey background
        xaxis=dict(
            title='Year',
            showgrid=True,
            gridcolor='lightgrey'
        ...
        yaxis=dict(
            title=f'Portfolio Value {denom}',
            tickprefix = denom,
            separatethousands=True,
            showgrid=True,
            gridcolor='lightgrey',
            rangemode='tozero'  # Prevent values going below 0
        )
    )

    st.write("")
    st.subheader("Portfolio Allocation Over Time During Drawdown")
    final_value = int(df_projection['End Value'].iloc[-1])
    withdrawal_abs = abs(int(annual_withdrawals))
    ...
    st.write(
    f'Given annual drawdowns of **{display_denom}{withdrawal_abs:,}** and selected asset allocation, '
